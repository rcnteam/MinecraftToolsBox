...
{
    /// <summary>
    /// EntitySelector.xaml 的交互逻辑
    /// </summary>
    public partial class EntitySelector : DockPanel
    {
        ...
        public void GenerateTarget()
        {
            ...
            {/*初始化所有参数*/
                ...
                if (entity_not.IsChecked == false)//检查实体目标是否反选
                {
                    ...
                    if (e == null || e == neutral || e == hostile || e == friend || e == BOSS || e == _throw || e == traffic || e == other || target.SelectedIndex != 3) ENTITY = "";
                    else
                    {
        ...
        private void Enable(object sender, RoutedEventArgs e)
        {
            ...
            else if (sender == entity_not)
            {
                if (entity_not.IsChecked == true)
                {
                    addE.IsEnabled = true;
                    delE.IsEnabled = true;
                    E.IsEnabled = true;
        ...
        {
            if (entity != null)
                if (target.SelectedIndex == 0) { entity.IsEnabled = false; ENABLE.Visibility = Visibility.Visible; }
                else { entity.IsEnabled = true; ENABLE.Visibility = Visibility.Collapsed; }
        }
        ...
        private void DataControl(object sender, RoutedEventArgs e)
        {
            ...
            if (b == addE)
            {
                ...
                if (t == null || t == neutral || t == hostile || t == friend || t == BOSS || t == _throw || t == traffic || t == other) return;
                string ENTI = "不选择选择类型为 " + t.Name + " 的实体(type=!" + t.Name + ")";
                for(int i = 0; i < E.Items.Count; i++)
                {
                    if ((string)E.Items[i] == ENTI) return;
                }
                E.Items.Add(ENTI);
            }
            if (b == delE) E.Items.Remove(E.SelectedItem);
        }
    }
}
